...
export async function registerRoutes(
  ...
  app.post("/api/customers", isAuthenticated, canEditDebtors, async (req, res) => {
    try {
  ...
  app.patch("/api/customers/:id", isAuthenticated, canEditDebtors, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.delete("/api/customers/cleanup", isAuthenticated, canEditDebtors, async (req, res) => {
    try {
  ...
  app.delete("/api/customers/:id", isAuthenticated, canEditDebtors, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  app.post("/api/customers/:id/bhb-sync", isAuthenticated, canEditDebtors, async (req, res) => {
    try {
      const { id } = req.params;
  ...
  // Get all internal users (admin, user, viewer - excludes customer accounts)
  app.get("/api/admin/users", isAuthenticated, isAdmin, async (req, res) => {
    try {
      const users = await storage.getInternalUsers();
  ...
  app.post("/api/admin/users", isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      if (role && !validRoles.includes(role)) {
        return res.status(400).json({ message: "Ungültige Rolle. Erlaubt: admin, user, viewer" });
      }

      // Validate password strength (min 10 chars)
      ...
      const user = await storage.createUser(username, password, displayName || username, role || "user");
  ...
  app.patch("/api/admin/users/:id", isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      // Prevent changing own role if it would remove the last admin
      if (currentUser.id === id && role && role !== "admin" && targetUser.role === "admin") {
        const adminCount = await storage.countAdmins();
        if (adminCount <= 1) {
          return res.status(400).json({ message: "Der letzte Admin-Benutzer kann seine Rolle nicht ändern" });
        }
      }

      // Prevent demoting someone else if they're the last admin
      if (targetUser.role === "admin" && role && role !== "admin") {
        const adminCount = await storage.countAdmins();
        if (adminCount <= 1) {
          return res.status(400).json({ message: "Der letzte Admin-Benutzer kann nicht herabgestuft werden" });
        }
      }

      const updated = await storage.updateUser(id, { displayName, role, password });
      if (!updated) {
        return res.status(404).json({ message: "Benutzer nicht gefunden" });
      }
  ...
  app.delete("/api/admin/users/:id", isAuthenticated, isAdmin, async (req, res) => {
    try {
      ...
      if (targetUser.role === "admin") {
        const adminCount = await storage.countAdmins();
        if (adminCount <= 1) {
          return res.status(400).json({ message: "Der letzte Admin-Benutzer kann nicht gelöscht werden" });
        }
      }
